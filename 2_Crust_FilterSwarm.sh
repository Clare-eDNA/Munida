...
# enter the directory
cd /nesi/nobackup/uoo02328/clare/Munida/LongCrust/filtering/Swarm/ACTUALfiltering || exit
...
awk 'BEGIN{FS=" "}{if(!/>/){print toupper($0)}else{print $1}}' Ampli-Full-Derep-5.fasta > Amplicons-Crust.fasta
#here's a full, uppercase file to SWARM
awk 'BEGIN{FS=" "}{if(!/>/){print toupper($0)}else{print $1}}' allsamples.fa > allsamples_Crust.fasta
...
swarm -f -z -t 4 -w allsample_OTU_reps.fasta Amplicons-Crust.fasta > /dev/null
...
#rename things
usearch -fastx_relabel Amplicons-Crust.fasta -prefix crustOTU. -fastaout crust_swarmOTUs_12oct20.fasta -keep_annots
# run a zotu output table
usearch -otutab allsamples_Crust.fasta -otus crust_swarmOTUs_12oct20.fasta -otutabout OTU-Crust-Swarm-labeled.txt
